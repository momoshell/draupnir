...
Loads routed entities + geometry for a revision, calls the passthrough producer,
and persists :class:`~app.models.revision_routed_length.RevisionRoutedLength` rows
with an idempotent INSERT ... ON CONFLICT DO NOTHING.
...
async def materialize_centerline_lengths(
    ...
    *,
    ...
    """Load routed entities, run the passthrough producer, and persist results.
    ...
    # Produce per-group centerlines via passthrough.
    centerlines = passthrough_centerlines(run_groups, inputs.geometry_by_entity_id)
