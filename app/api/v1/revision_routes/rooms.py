...
The room-resolution internals (``_resolve_rooms``, ``_resolve_rooms_with_family``, and
their loader/adapter helpers) moved to ``app.interpretation.room_resolution`` (#852);
this module keeps only the FastAPI route glue plus the Query-facing ``RoomScope`` /
``_MAX_NESTING_DEPTH`` (route concerns, not resolver internals).
"""
...
from app.interpretation.room_resolution import _resolve_rooms, _resolve_rooms_with_family
from app.interpretation.rooms import Room, _smallest_containing_room
